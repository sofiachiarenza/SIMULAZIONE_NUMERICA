...
double Popolazione::Fitness(rowvec cromo){
    ...
    for (int i = 0 ; i < m_citta - 2 ; i++ ){
        ...
        cost += sqrt( pow( positions.at(index1 , 0)-positions.at(index2 , 0) , 2. )
                + pow( positions.at(index1 , 1)-positions.at(index2 , 1) , 2. ) ) ;

    }
    cost += sqrt( pow( positions.at( index2 , 0)-positions.at(0 , 0) , 2. ) + pow(positions.at( index2 , 1)-positions.at(0 , 1) , 2. ) );
